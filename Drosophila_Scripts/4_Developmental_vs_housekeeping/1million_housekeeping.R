...
all_data <- get(load("/home/jw18713/P1_Full_Run/Rda_objects/Tiled_genomes/S2_dCP_hkCP_tiled.Rda")) #Loading in full S2 dataset as all_data
...
pos_ratio <- sum(all_data$hkCP_binary)/length(all_data)
...
for (i in seq_along(chr_split)){
  pos_neg_split <- split(chr_split[[i]], chr_split[[i]]$hkCP_binary)
...
save(chr_split_resampled, file = "~/1million_housekeeping.Rda")
write.csv(as.data.frame(chr_split_resampled), file = "~/1million_housekeeping.csv", row.names = F)
