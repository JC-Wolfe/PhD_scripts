...
no_expression_BG3P <- BG3_common_prox[sapply(BG3_common_prox, length) == 0]

# And which of these are expressed?
enhancers_contacted_BG3P <- BG3_common_prox[sapply(BG3_common_prox, length) > 0]
...
# Now which of these don't have any expresion?
no_expression_BG3D <- BG3_common_dist[sapply(BG3_common_dist, length) == 0]

# And which of these are expressed?
enhancers_contacted_BG3D <- BG3_common_dist[sapply(BG3_common_dist, length) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2P <- S2_common_prox[sapply(S2_common_prox, length) == 0]

# And which of these are expressed?
enhancers_contacted_S2P <- S2_common_prox[sapply(S2_common_prox, length) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2D <- S2_common_dist[sapply(S2_common_dist, length) == 0]

# And which of these are expressed?
enhancers_contacted_S2D <- S2_common_dist[sapply(S2_common_dist, length) > 0]
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS3/FigureS3D_max.pdf",
...
hist(log2(expression_max_BG3D+1), main="BG3 Enhancers\nMax Contacted Expression",
  ...
  ylim = c(0,300), breaks = 16, ylab = NA, xlim= c(0,16))
...
# Ok, so this is my histogram then
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS3/FigureS3D_mean.pdf",
width = 14, height = 12, pointsize = 14)
par(mar=c(4.9,5.5,5.5,8), mfrow=c(2,2), cex = 1.2)
par(xpd = NA)
