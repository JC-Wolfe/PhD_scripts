...
no_expression_BG3P <- BG3_starr_prox[sapply(BG3_starr_prox, length) == 0]

# And which of these are expressed?
enhancers_contacted_BG3P <- BG3_starr_prox[sapply(BG3_starr_prox, length) > 0]
...
# Now which of these don't have any expresion?
no_expression_BG3D <- BG3_starr_dist[sapply(BG3_starr_dist, length) == 0]

# And which of these are expressed?
enhancers_contacted_BG3D <- BG3_starr_dist[sapply(BG3_starr_dist, length) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2P <- S2_starr_prox[sapply(S2_starr_prox, length) == 0]

# And which of these are expressed?
enhancers_contacted_S2P <- S2_starr_prox[sapply(S2_starr_prox, length) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2D <- S2_starr_dist[sapply(S2_starr_dist, length) == 0]

# And which of these are expressed?
enhancers_contacted_S2D <- S2_starr_dist[sapply(S2_starr_dist, length) > 0]
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS4/FigureS4_max.pdf",
...
hist(log2(expression_max_BG3D+1), main="BG3 Enhancers\nMax Contacted Expression",
  ...
  ylim = c(0,300), breaks = seq(0,16,by=1), ylab = NA)
axis(1, at=seq(0,16,by=4), labels=c(0,2^seq(4,16, by = 4)))
axis(2, at=seq(0,300, by = 100), labels = seq(0,300, by = 100), las=2)

hist(log2(expression_max_BG3P+1), col="#D55E0080", add=T, breaks = seq(0,16,by=1))
...
hist(log2(expression_max_S2P+1), col="#D55E0080", add=T, breaks = seq(0,16,by=1))
...
hist(log2(expression_max_BG3_bg_p+1), col="#E69F0080", add=T, breaks = seq(0,16,by=1))
...
hist(log2(expression_max_S2_bg_p+1), col="#E69F0080", add=T, breaks = seq(0,16,by=1))
...
# Ok, so this is my histogram then
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS4/FigureS4_mean.pdf",
width = 14, height = 12, pointsize = 14)
par(mar=c(4.9,5.5,5.5,8), mfrow=c(2,2), cex = 1.2)
par(xpd = NA)
...
legend(16.5,200,c("Distal", "Proximal"), fill=c("#0072B280", "#D55E0080"), bty='n')
