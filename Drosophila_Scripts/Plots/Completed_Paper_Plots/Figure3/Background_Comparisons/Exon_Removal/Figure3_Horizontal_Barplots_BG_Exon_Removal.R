...
BG3_put_prox <- exon_removal(BG3_put_prox, exons)
BG3_put_dist <- exon_removal(BG3_put_dist, exons)
BG3_bg_prox <- exon_removal(BG3_bg_prox, exons)
BG3_bg_dist <- exon_removal(BG3_bg_dist, exons)

S2_put_prox <- exon_removal(S2_put_prox, exons)
S2_put_dist <- exon_removal(S2_put_dist, exons)
...
no_expression_BG3P <- BG3_put_prox[lengths(BG3_put_prox) == 0]

# And which of these are expressed?
enhancers_contacted_BG3P <- BG3_put_prox[lengths(BG3_put_prox) > 0]
...
# Now which of these don't have any expresion?
no_expression_BG3D <- BG3_put_dist[lengths(BG3_put_dist) == 0]

# And which of these are expressed?
enhancers_contacted_BG3D <- BG3_put_dist[lengths(BG3_put_dist) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2P <- S2_put_prox[lengths(S2_put_prox) == 0]

# And which of these are expressed?
enhancers_contacted_S2P <- S2_put_prox[lengths(S2_put_prox) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2D <- S2_put_dist[lengths(S2_put_dist) == 0]

# And which of these are expressed?
enhancers_contacted_S2D <- S2_put_dist[lengths(S2_put_dist) > 0]
...
fish_S2_put_dist <- S2_em[1,]
fish_S2_bg_dist <- S2_em[2,]
fish_S2_put_prox <- S2_em[3,]
fish_S2_bg_prox <- S2_em[4,]



fisher.test(rbind(fish_S2_put_prox, fish_S2_put_dist))
fisher.test(rbind(fish_S2_put_prox, fish_S2_bg_prox))
fisher.test(rbind(fish_S2_put_dist, fish_S2_bg_dist))
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/Figure3/Figure3_Horiz_BG_noexons.pdf",
