...
load("/home/jw18713/rerun_glists/common/BG3_common_dist.Rda")
load("/home/jw18713/rerun_glists/common/BG3_common_prox.Rda")
...
load("/home/jw18713/rerun_glists/common/S2_common_dist.Rda")
load("/home/jw18713/rerun_glists/common/S2_common_prox.Rda")
...
exons <- reduce(ann_genome[ann_genome$annotations == "exon"])

exon_removal <- function(x, exons){
  regions <- names(x)
  chrs <- unlist(strsplit(regions, ":"))[c(T,F)]
  ranges <- unlist(strsplit(regions, ":"))[c(F,T)]
  starts <- as.numeric(unlist(strsplit(ranges, "-"))[c(T,F)])
  ends <- as.numeric(unlist(strsplit(ranges, "-"))[c(F,T)])
  grcompare <- GRanges(seqnames = chrs, ranges = IRanges(start = starts, end = ends))
  over <- findOverlaps(grcompare, exons)
  listout <- x[-unique(queryHits(over))]
  return(listout)
}

BG3_common_prox <- exon_removal(BG3_common_prox, exons)
BG3_common_dist <- exon_removal(BG3_common_dist, exons)
BG3_bg_prox <- exon_removal(BG3_bg_prox, exons)
BG3_bg_dist <- exon_removal(BG3_bg_dist, exons)

S2_common_prox <- exon_removal(S2_common_prox, exons)
S2_common_dist <- exon_removal(S2_common_dist, exons)
S2_bg_prox <- exon_removal(S2_bg_prox, exons)
S2_bg_dist <- exon_removal(S2_bg_dist, exons)


# Proximal BG3 (BG3P)

# Now which of these don't have any expresion?
no_expression_BG3P <- BG3_common_prox[lengths(BG3_common_prox) == 0]

# And which of these are expressed?
enhancers_contacted_BG3P <- BG3_common_prox[lengths(BG3_common_prox) > 0]
...
# Now which of these don't have any expresion?
no_expression_BG3D <- BG3_common_dist[lengths(BG3_common_dist) == 0]

# And which of these are expressed?
enhancers_contacted_BG3D <- BG3_common_dist[lengths(BG3_common_dist) > 0]
...
common_dist <- BG3_em[1,]
bg_dist <- BG3_em[2,]
common_prox <- BG3_em[3,]
bg_prox <- BG3_em[4,]


fisher.test(rbind(common_prox, common_dist))
fisher.test(rbind(common_prox, bg_prox))
fisher.test(rbind(common_dist, bg_dist))
...
# Now which of these don't have any expresion?
no_expression_S2P <- S2_common_prox[lengths(S2_common_prox) == 0]

# And which of these are expressed?
enhancers_contacted_S2P <- S2_common_prox[lengths(S2_common_prox) > 0]
...
# Now which of these don't have any expresion?
no_expression_S2D <- S2_common_dist[lengths(S2_common_dist) == 0]

# And which of these are expressed?
enhancers_contacted_S2D <- S2_common_dist[lengths(S2_common_dist) > 0]
...
colnames(BG3_plot) <- c("Distal Only", "Proximal")
...
colnames(S2_plot) <- c("Distal Only", "Proximal")
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS3/FigureS3_Horiz_BG.pdf",
...
par(xpd = NA)
  ...
  barplot(BG3_plot[2:1,2:1], beside=T, horiz = T, col = cbbPalette[c(2,7,3,6)],
    ...
    # common proximal and proximal bg
    ...
    # common distal only and common proximal
