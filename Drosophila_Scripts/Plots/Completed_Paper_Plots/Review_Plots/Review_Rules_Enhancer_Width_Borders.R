...
rules <- read.csv("Border_Rules.csv",
  stringsAsFactors = F)
...
features <- read.csv("Border_Features.csv", stringsAsFactors = F)
...
text(seq_along(marks_pos_scored),y = length(pos_df[,1]) + 0.5, srt = 315, adj = 1,labels = marks_pos_scored, xpd = TRUE,cex=0.75)
...
legend(31, length(pos_df[,1])*0.7, legend = c("High", "Medium", "Low"),
...
axis(3, at=seq(1,length(marks), by = 1), labels=F)
text(seq_along(marks_neg_scored),y =length(neg_df[,1]) + 0.75, srt = 315, adj = 1,labels = marks_neg_scored, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(neg_df$Id), by = 1), labels=neg_df$Id, las=2)

title(ylab = "Rule ID", cex.lab = 1, line = 3)
title(main = "Putative Enhancer Rules", cex.main = 2, line = 5)

legend(31, length(neg_df[,1])*0.7, legend = c("High", "Medium", "Low"),
       fill = c("forestgreen", "orange", "red"),
       title = "Fuzzy Membership",
       bty="n")
