...
rules <- read.csv("With_SA_Rules.csv",
  stringsAsFactors = F)
...
features <- read.csv("With_SA_Features.csv", stringsAsFactors = F)
...
text(seq_along(marks_pos_scored),y = length(pos_df[,1]) + 0.6, srt = 315, adj = 1,labels = marks_pos_scored, xpd = TRUE,cex=0.75)
...
axis(3, at=seq(1,length(marks), by = 1), labels=F)
text(seq_along(marks_neg_scored),y =length(neg_df[,1]) + 0.7, srt = 315, adj = 1,labels = marks_neg_scored, xpd = TRUE,cex=0.75)
