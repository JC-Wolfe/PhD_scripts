...
# loading p_under5k (promoter contacts over 5kb away)
...
proximal_enhancers <- proximal_enhancers[width(proximal_enhancers) > 50 & width(proximal_enhancers) < 1000] # Just checking Standard enhancers
...
list_proximal_enhancers <- split(proximal_enhancers, as.factor(proximal_enhancers))
...
old <- Sys.time()
proximal_enhancers_glist <- lapply(list_proximal_enhancers, expression_extraction, expression_data = contacted_expressed)
new <- Sys.time()
print(old - new)
save(proximal_enhancers_glist, file = "/home/jw18713/Project1/Paper_Rda_Stuff/BG3/proximal_Standard_enhancer_glist.Rda")

# Now which of these don't have any expresion?
no_expression <- proximal_enhancers_glist[sapply(proximal_enhancers_glist, length) == 0]

# And which of these are expressed?
proximal_enhancers_contacted <- proximal_enhancers_glist[sapply(proximal_enhancers_glist, length) > 0]
...
medians_plot <- sapply(proximal_enhancers_contacted, medianExp)
...
max_to_plot <- sapply(proximal_enhancers_contacted, maxExp)

no_expression_median <- c(no_expression, proximal_enhancers_contacted[medians_plot == 0])
no_expression_max <- c(no_expression, proximal_enhancers_contacted[max_to_plot == 0])
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/Figure4/BG3/BG3_Proximal_Standard_Enhancers_Zero_Expression_Barplot_Median.pdf")
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/Figure4/BG3/BG3_Proximal_Standard_Enhancers_Zero_Expression_Barplot_Max.pdf")
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/Figure4/BG3/BG3_Proximal_Standard_Enhancer_Expression_Histogram.pdf")
hist(log2(expression_max+1), main="BG3 Proximal Standard Enhancers\nMax Contacted Expression", xlab="Expression FPKM", col="lightblue", cex.main=2, cex.lab=1.5, axes=F, xlim = c(0,15), ylim = c(0,3000))
