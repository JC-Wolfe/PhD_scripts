...
BG3_enhancers_neither <- get(load("~/Archive_Year1/BG3_novel_neither.Rda"))
BG3_SE_neither <- BG3_enhancers_neither[width(BG3_enhancers_neither) >= 1000]
BG3_proximal_enhancers <- get(load("~/Archive_Year1/BG3_novel_proximal.Rda"))
BG3_proximal_SE <- BG3_proximal_enhancers[width(BG3_proximal_enhancers) >= 1000]
BG3_distal_enhancers <- get(load("~/Archive_Year1/BG3_novel_distal_all.Rda"))
BG3_distal_SE <- BG3_distal_enhancers[width(BG3_distal_enhancers) >= 1000]


v1 <- generateTripleVenn(BG3_distal_SE, BG3_proximal_SE, BG3_SE_neither, c("Distal\nPromoter", "Proximal\nPromoter", "Neither"),
...
v2 <- generateTripleVenn(S2_distal_SE, S2_proximal_SE, S2_SE_neither, c("Distal\nPromoter", "Proximal\nPromoter", "Neither"),
                           cols=c(cbbPalette[6], cbbPalette[7], cbbPalette[8]),
                           cat.pos = c(-60, 60, 180),
                           cat.dist = c(0.125, 0.125, 0.05),
                           human = F)

pdf("~/Project1/Paper_Plots/FigureS2/FigureS2A.pdf", width=8, height=5,pointsize = 10);
