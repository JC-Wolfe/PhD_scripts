...
load("/home/jw18713/Project1/Paper_Rda_Stuff/BG3/BG3_distal_only_glist_SE.Rda")
load("/home/jw18713/Project1/Paper_Rda_Stuff/BG3/BG3_proximal_enhancers_glist_SE.Rda")

load("/home/jw18713/Project1/Paper_Rda_Stuff/S2/S2_distal_only_glist_SE.Rda")
load("/home/jw18713/Project1/Paper_Rda_Stuff/S2/S2_proximal_enhancer_glist_SE.Rda")
...
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS2/FigureS2D.pdf",
...
hist(log2(expression_max_BG3D+1), main="BG3 Enhancers\nMax Contacted Expression",
  ...
  ylim = c(0,200), breaks = seq(0,16, by = 1), ylab = NA)
axis(1, at=seq(0,16,by=4), labels=c(0,2^seq(4,16, by = 4)))
axis(2, at=seq(0,200, by = 50), labels = seq(0,200, by = 50), las=2)

hist(log2(expression_max_BG3P+1), col="#D55E0080", add=T, breaks = 16)

legend(16.5,1500,c("Distal", "Proximal"), fill=c("#0072B280", "#D55E0080"), bty='n')
...
# Ok, so this is my histogram then
pdf(file = "/home/jw18713/Project1/Paper_Plots/FigureS2/FigureS3Dmeans.pdf",
width = 14, height = 6, pointsize = 14)
...
axis(1, at=seq(0,16,by=4), labels=c(0,2^seq(4,16, by = 4)))
axis(2, at=seq(0,300, by = 50), labels = seq(0,300, by = 50), las=2)

hist(log2(expression_means_BG3P+1), col="#D55E0080", add=T, breaks = seq(0,16,by=1))

legend(16.5,175,c("Distal", "Proximal"), fill=c("#0072B280", "#D55E0080"), bty='n')
