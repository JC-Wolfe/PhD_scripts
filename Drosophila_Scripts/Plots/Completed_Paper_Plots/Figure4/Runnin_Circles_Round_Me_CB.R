...
for (i in seq_along(score_buffer)){
  ...
  M1_score <- 1 * length(which(M1_shared$C1==cbbPalette[3]))+
              -1 * length(which(M1_shared$C1=="red"))

  M2_shared <- pos_df[which(pos_df$M2==marks[i]),]
  M2_score <- 1 * length(which(M2_shared$C2==cbbPalette[3]))+
              -1 * length(which(M2_shared$C2=="red"))

  M3_shared <- pos_df[which(pos_df$M3==marks[i]),]
  M3_score <- 1 * length(which(M3_shared$C3==cbbPalette[3]))+
...
save(marks_pos_scored, file = "Positive_ordered_marks.Rda")
...
legend(27, length(pos_df[,1])*0.75, legend = c("High", "Medium", "Low"),
       fill = c(cbbPalette[3], "orange", "red"),
       bty="n")
dev.off()
...
# CUSTOM RULES
# Everything below here has been moved in for convenience
...
plotting_df$M1 <- sapply(rules_df$M1, mark_number, marks)
plotting_df$L1 <- as.character(rules_df$L1)
plotting_df$M2 <- sapply(rules_df$M2, mark_number, marks)
plotting_df$L2 <- as.character(rules_df$L2)
plotting_df$M3 <- sapply(rules_df$M3, mark_number, marks)
plotting_df$L3 <- as.character(rules_df$L3)
plotting_df$Classification <- rules_df$Classification

plotting_df$L1[plotting_df$L1=="high"] <- cbbPalette[3]
plotting_df$L1[plotting_df$L1=="medium"] <- "orange"
plotting_df$L1[plotting_df$L1=="low"] <- "red"

plotting_df$L2[plotting_df$L2=="high"] <- cbbPalette[3]
plotting_df$L2[plotting_df$L2=="medium"] <- "orange"
plotting_df$L2[plotting_df$L2=="low"] <- "red"

plotting_df$L3[plotting_df$L3=="high"] <- cbbPalette[3]
plotting_df$L3[plotting_df$L3=="medium"] <- "orange"
plotting_df$L3[plotting_df$L3=="low"] <- "red"

plotting_df$L2[plotting_df$L2 == ""] <- NA
plotting_df$L3[plotting_df$L3 == ""] <- NA

pos_df <- plotting_df[which(plotting_df$Classification==1),]
neg_df <- plotting_df[which(plotting_df$Classification==0),]
...
axis(3, at=seq(1,26, by = 1), labels=F)
text(seq_along(marks),y = length(pos_df[,1]) + 0.75, srt = 315, adj = 1,labels = marks, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(pos_df$ID), by = 1), labels=pos_df$ID, las=2)
...
text(seq_along(marks),y =length(neg_df[,1]) + 0.9, srt = 315, adj = 1,labels = marks, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(neg_df$ID), by = 1), labels=neg_df$ID, las=2)
