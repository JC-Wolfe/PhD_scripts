...
load("~/Paper_replots/Objects/Expression_lists/S2_background_proximal.Rda")
load("~/Paper_replots/Objects/Expression_lists/S2_background_distal_only.Rda")
load("~/Paper_replots/Objects/Expression_lists/BG3_background_proximal.Rda")
load("~/Paper_replots/Objects/Expression_lists/BG3_background_distal_only.Rda")
load("~/Paper_replots/Objects/Expression_lists/S2_putative_proximal.Rda")
load("~/Paper_replots/Objects/Expression_lists/S2_putative_distal_only.Rda")
load("~/Paper_replots/Objects/Expression_lists/BG3_putative_proximal.Rda")
load("~/Paper_replots/Objects/Expression_lists/BG3_putative_distal_only.Rda")

cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442",
  "#0072B2", "#D55E00", "#CC79A7")

# Ok, so this is my histogram then
pdf(file = "~/Paper_replots/Plots/Figure3D.pdf",
...
statsmatrix_3D <- matrix(0, 2, 2, dimnames = list(c("Putative", "Background"),
  c("BG3", "S2")))

# Stats for figure legends
statsmatrix_3D[1,1] <- wilcox.test(log2(BG3_putative_distal_only$max_Expression+1),
  log2(BG3_putative_proximal$max_Expression+1))$p.value

statsmatrix_3D[1,2] <- wilcox.test(log2(S2_putative_distal_only$max_Expression+1),
  log2(S2_putative_proximal$max_Expression+1))$p.value

statsmatrix_3D[2,1] <- wilcox.test(log2(BG3_background_distal_only$max_Expression+1),
  log2(BG3_background_proximal$max_Expression+1))$p.value

statsmatrix_3D[2,2] <- wilcox.test(log2(S2_background_distal_only$max_Expression+1),
  log2(S2_background_proximal$max_Expression+1))$p.value

write.csv(statsmatrix_3D, file = "~/Paper_replots/Stats/3D_stats.csv")
