...
# Plotting Figure 3B
pdf(file = "~/Paper_replots/Plots/Figure3/Figure3B_redone.pdf",
width = 20, height = 8, pointsize = 14)
par(mar=c(5,4.5,5,7), mfrow=c(1,2), cex = 1.2)
par(xpd = NA)

      # BG3 proximal
      hist(log2(BG3_putative_PR), axes=F, main="BG3 Putative Enhancer Widths",
        xlab="Width (bp)", col="#D55E0080", cex.main=2, cex.lab=1.5, xlim = c(3,14), ylim = c(0,2000),
        breaks = 20, ylab=NA)
      ...
      #S2 proximal
      hist(log2(S2_putative_PR), axes=F, main="S2 Putative Enhancer Widths",
...
statsmatrix_3B <- matrix(0, 1, 2, dimnames = list(c("Putative Length"), c("BG3", "S2")))

# Stats for figure legends
statsmatrix_3B[1,1] <- wilcox.test(BG3_putative_PR, BG3_putative_DO)$p.value
statsmatrix_3B[1,2] <- wilcox.test(S2_putative_PR, S2_putative_DO)$p.value

write.csv(statsmatrix_3B, file = "~/Paper_replots/Stats/3B_stats.csv")

# Plotting Figure S3B
pdf(file = "~/Paper_replots/Plots/Figure3/FigureS4A_redone.pdf",
width = 18, height = 8, pointsize = 14)
par(mar=c(5,5.5,5,7), mfrow=c(1,2), cex = 1.2)
par(xpd = NA)

      # BG3 proximal
      hist(log2(BG3_common_PR), axes=F, main="BG3 Common Enhancer Widths",
      ...
      #S2 proximal
      hist(log2(S2_common_PR), axes=F, main="S2 Common Enhancer Widths",
        xlab="Width (bp)", col="#D55E0080", cex.main=2, cex.lab=1.5, xlim = c(3,14), ylim = c(0,300),
        breaks = 20, ylab=NA)
          axis(1, at=c(log2(10), log2(50), log2(150), log2(400), log2(1000), log2(4000), log2(16000)),
              labels=c(10, 50, 150, 400, 1000, 4000, 16000))
...
# Stats for figure legends
statsmatrix_S4A <- matrix(0, 1, 2, dimnames = list(c("Common Length"), c("BG3", "S2")))

# Stats for figure legends
statsmatrix_S4A[1,1] <- wilcox.test(BG3_common_PR, BG3_common_DO)$p.value
statsmatrix_S4A[1,2] <- wilcox.test(S2_common_PR, S2_common_DO)$p.value

write.csv(statsmatrix_S4A, file = "~/Paper_replots/Stats/S4A_stats.csv")
