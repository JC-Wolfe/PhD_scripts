...
setwd("~/Human_Datasets/H9/Pradeep")
rules <- read.csv("Pradeep_rules.csv",
  stringsAsFactors = F)
...
features <- read.csv("Pradeep_features.csv", stringsAsFactors = F)
...
plotting_df$C1[plotting_df$C1=="0"] <- "black"
...
plotting_df$C2[plotting_df$C2=="0"] <- "black"
...
plotting_df$C3[plotting_df$C3=="0"] <- "black"
...
pos_plot_names[pos_plot_names == "ATAC"] <- "ATAC-seq"
pos_plot_names <- pos_plot_names[c(-5, -24, -25, -35)]
...
neg_plot_names[neg_plot_names == "ATAC"] <- "ATAC-seq"
neg_plot_names <- neg_plot_names[c(-14, -15, -26, -34)]

# And plotting both

setwd("~/Human_Datasets/Plots/Figure4")
pdf("Pradeep_enhancers.pdf",
  width = 12, height = 8, pointsize = 14)
...
text(seq_along(pos_plot_names),y = length(pos_df[,1]) + 1.25, srt = 315, adj = 1,labels = pos_plot_names, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(pos_df$Id), by = 1), labels=pos_df$Id, las=2)

title(ylab = "Rule ID", cex.lab = 1, line = 3)
title(main = "H9 Enhancer Rules", cex.main = 2, line = 5)

legend(35, length(pos_df[,1])*0.8, legend = c("High", "Medium", "Low", "Zero"),
       fill = c("forestgreen", "orange", "red", "black"),
...
axis(3, at=seq(1,length(marks), by = 1), labels=F)
text(seq_along(neg_plot_names),y =length(neg_df[,1]) + 0.60, srt = 315, adj = 1,labels = neg_plot_names, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(neg_df$Id), by = 1), labels=neg_df$Id, las=2)

title(ylab = "Rule ID", cex.lab = 1, line = 3)
title(main = "H9 Non-Enhancer Rules", cex.main = 2, line = 5)

legend(35, length(neg_df[,1]), legend = c("High", "Medium", "Low", "Zero"),
       fill = c("forestgreen", "orange", "red", "black"),
       title = "Fuzzy Membership",
       bty="n")
