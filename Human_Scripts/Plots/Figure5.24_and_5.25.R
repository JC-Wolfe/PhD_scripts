...
setwd("~/Human_Datasets/HeLa/Rules")
rules <- read.csv("HeLa_strict_rules.csv",
  stringsAsFactors = F)
...
features <- read.csv("HeLa_strict_features.csv", stringsAsFactors = F)
...
pdf("HeLa_strict_enhancers.pdf",
  width = 12, height = 18, pointsize = 14)
...
text(seq_along(pos_plot_names),y = length(pos_df[,1]) + 1.5, srt = 315, adj = 1,labels = pos_plot_names, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(pos_df$Id), by = 1), labels=pos_df$Id, las=2)

title(ylab = "Rule ID", cex.lab = 1, line = 3)
title(main = "HeLa Enhancer Rules", cex.main = 2, line = 5)

legend(16, length(pos_df[,1])*0.6, legend = c("High", "Medium", "Low"),
...
pdf("HeLA_strict_non-enhancers.pdf",
  width = 12, height = 5, pointsize = 14)
par(mar=c(3,5.5,8.5,9), cex = 1.2)
par(xpd = NA)
...
axis(3, at=seq(1,length(marks), by = 1), labels=F)
text(seq_along(neg_plot_names),y =length(neg_df[,1]) + 0.6, srt = 315, adj = 1,labels = neg_plot_names, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(neg_df$Id), by = 1), labels=neg_df$Id, las=2)

title(ylab = "Rule ID", cex.lab = 1, line = 3)
title(main = "HeLa Non-Enhancer Rules", cex.main = 2, line = 5)

legend(16, 1.7, legend = c("High", "Medium", "Low", "Zero"),
