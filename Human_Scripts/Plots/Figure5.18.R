...
setwd("~/Human_Datasets/H9/csv_files/Rules/450_31_model")
rules <- read.csv("450_31_rules.csv",
  stringsAsFactors = F)
...
features <- read.csv("450_31_features.csv", stringsAsFactors = F)
...
legend(32, length(pos_df[,1])*0.7, legend = c("High", "Medium", "Low"),
...
pdf("H9_450_31_non-enhancers.pdf",
  width = 12, height = 8, pointsize = 14)
par(mar=c(3,5.5,8.5,8), cex = 1.2)
par(xpd = NA)
...
legend(32, length(neg_df[,1])*0.7, legend = c("High", "Medium", "Low", "Zero"),
