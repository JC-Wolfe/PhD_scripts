...
H1 <- H1_XAI[H1_XAI$Conf_Perc_1 >= 0.8]
H1 <- reduce(H1, min.gapwidth = 1000)
H1 <- H1[width(H1) >= 200]
...
labels <- c(paste0("STARR-seq\n(", length(STARR), ")"),
  paste0("H9\n(", length(H9), ")"),
  paste0("H1\n(", length(H1), ")"))

v1 <- generateTripleVenn(STARR, H9, H1, c(labels),
...
pdf("/home/jw18713/Human_Datasets/Plots/Supp/H1_H9_STARR-seq_VD(width).pdf", width=9, height=10,pointsize = 14);
