...
over_thresh <- H1_XAI[H1_XAI$Conf_Perc_1 >= 0.8]

rdat <- GRanges(seqnames = (seqnames(H1_XAI)), ranges = IRanges(start = start(H1_XAI),
end = end(H1_XAI)))
rdat$readsM <- H1_XAI$Conf_Perc_1
rdat$readsN <- 1
rdat$context <- "CG"
...
H1_reduction_matrix <- matrix(c(counts, averages, widths), 3, 7, byrow = T,
  dimnames = list(c("Count", "Mean_Conf_Perc_1", "Mean Width"),
  c("0", "500", "1000", "1500", "2000", "2500", "3000")))

save(H1_reduction_matrix,
  file = "~/Human_Datasets/H1/Processed_datasets/H1_reduction_matrix.Rda")
