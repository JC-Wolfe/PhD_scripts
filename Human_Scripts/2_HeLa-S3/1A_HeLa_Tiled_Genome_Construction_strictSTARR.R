...
STARR_t <- read.csv("~/Human_Datasets/HeLa/STARR_seq/peaks_inhibitor_correctedEnrichment4_supp.table3.tsv",
  sep = "\t")

STARR <- GRanges(seqnames = STARR_t$seqnames, ranges = IRanges(
  start = STARR_t$start,
  end = STARR_t$end))
...
save(HeLa_tiled, file = "~/Human_Datasets/HeLa/DMRcaller_with_STARR_DNAse_strict.Rda")
...
save(HeLa_tiled, file = "~/Human_Datasets/HeLa/DMRcaller_norm_tiled_DNAse_strict.Rda")
